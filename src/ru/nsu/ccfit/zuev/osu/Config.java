...
public class Config {
    ...
    private static boolean useDither = true;//Dither技术，类似于量化压缩
    ...
    public static void loadConfig(final Context context) {
        ...
        final SharedPreferences prefs = PreferenceManager
                .getDefaultSharedPreferences(context);//临时保存一些变量的时候会用到SharedPreferences
        String s;
        // graphics
        ...
        s = prefs.getString("vbroffset", "50");//不是很懂这个vbr，可变比特率编码
        vbrOffset = 0;
        try {
            final int off = Integer.parseInt(s);
            vbrOffset = off;
        ...
        if (onlineDeviceID == null) {//随机一个UUID作为onlineDeviceID
